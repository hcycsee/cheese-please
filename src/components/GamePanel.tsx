...
import { gameRoomKey, type GameSession } from "@/lib/game";
...
export default function GamePanel({
  ...
  useEffect(() => {
    ...
    function onState(payload: { key: string; session: GameSession | null }) {
  ...
  function start() {
    getSocket().emit("game:start", { mode, targetId });
  }
  function join() {
  ...
  if (!session) {
    return (
      <div className="flex items-center justify-between border-b border-stone-200 px-5 py-2">
        ...
        <button className="btn-secondary text-xs" onClick={start}>
          🎮 Start a game
        </button>
      </div>
    );
  ...
  return (
    <div className="border-b border-stone-200 bg-stone-50 px-5 py-3">
      <div className="mb-2 flex items-center justify-between">
        <p className="text-sm font-semibold">🎮 Tile Rush</p>
