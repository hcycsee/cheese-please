...
export default function GamePanel({
  ...
  function leaveGame() {
    if (!window.confirm("Leave this game? It'll end for everyone in this chat.")) return;
    getSocket().emit("game:leave", { mode, targetId });
  }
  ...
  return (
    <div className="relative border-b border-stone-200 bg-stone-50 px-5 py-3">
      <button
        type="button"
        onClick={leaveGame}
        aria-label="Leave game"
        title="Leave game"
        className="absolute right-2 top-2 flex h-6 w-6 items-center justify-center rounded-full text-stone-400 hover:bg-stone-200 hover:text-stone-700"
      >
        ✕
      </button>
